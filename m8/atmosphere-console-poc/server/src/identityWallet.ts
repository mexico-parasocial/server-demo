...
const ineIssuerKey = generateKeyPairSync('ed25519')
const demoWalletKey = generateKeyPairSync('ed25519')
...
export const TRUSTED_ISSUERS: M8TrustedIssuer[] = [
  {
    ...
    country: 'MX',
    status: 'active',
    publicKeyPem: ineIssuerKey.publicKey.export({ type: 'spki', format: 'pem' }).toString(),
...
function signPayload(payload: string, privateKey: typeof ineIssuerKey.privateKey) {
...
export function createDemoWalletPresentation(params: {
  ...
  const credential: M8IdentityCredential = {
    ...unsignedCredential,
    signature: signPayload(signedCredentialPayload(unsignedCredential), ineIssuerKey.privateKey),
  }
  ...
  const unsignedPresentation: Omit<M8WalletPresentation, 'signature'> = {
    ...
    credential,
    disclosedClaims,
    devicePublicKey: demoWalletKey.publicKey.export({ type: 'spki', format: 'pem' }).toString(),
    issuedAt: nowIso(),
  ...
  return {
    ...
    signature: signPayload(signedPresentationPayload(unsignedPresentation), demoWalletKey.privateKey),
  }
}
