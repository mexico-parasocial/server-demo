...
PDS_DIR="${PARA_ROOT}/dev-env/node_modules/@atproto/pds"
DEV_INFRA="${PARA_ROOT}/dev-env/dev-infra"
...
if [ ! -d "${PDS_DIR}" ]; then
  ...
  echo "   Run 'pnpm install' in ${PARA_ROOT} first."
  exit 1
fi
...
cd "${PDS_DIR}"
"${DEV_INFRA}/with-test-redis-and-db.sh" jest "$@"
