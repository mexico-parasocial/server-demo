/**
 * mock-server-seaweedfs.ts
 *
 * Same as mock-server.ts, but forces the PDS to use SeaweedFS S3
 * for blob storage instead of a temp disk directory.
 *
 * Usage:
 *   SEAWEEDFS_S3_ENDPOINT=http://localhost:8333 \
 *   ts-node dev-env/mock-server-seaweedfs.ts
 */

import {createServer as createHTTPServer} from 'node:http'
import {parse} from 'node:url'

import {createServer, type TestPDS} from './test-pds-seaweedfs'
...
createHTTPServer(async (req, res) => {
  ...
  try {
    ...
    await server?.close()
    console.log('Starting new server (SeaweedFS S3 blobstore)')
...
console.log('Mock server manager (SeaweedFS S3) listening on 1986')
