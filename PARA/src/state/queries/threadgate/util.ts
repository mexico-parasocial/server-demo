...
import {MOCK_COMMUNITY_LIST_URI, MOCK_PARTY_LIST_URI} from '#/lib/constants'
  ...
  const settings: ThreadgateAllowUISetting[] = threadgate.allow
    .map(allow => {
      ...
      if (AppBskyFeedThreadgate.isMentionRule(allow)) {
        ...
        if (allow.list === MOCK_COMMUNITY_LIST_URI) {
          setting = {type: 'community'}
        } else if (allow.list === MOCK_PARTY_LIST_URI) {
          setting = {type: 'party'}
        } else {
...
export function threadgateAllowUISettingToAllowRecordValue(
  ...
  if (!threadgate.find(v => v.type === 'nobody')) {
    for (const rule of threadgate) {
      ...
      } else if (rule.type === 'community') {
        allow.push({
          $type: 'app.bsky.feed.threadgate#listRule',
          list: MOCK_COMMUNITY_LIST_URI,
        })
      } else if (rule.type === 'party') {
        allow.push({
          $type: 'app.bsky.feed.threadgate#listRule',
          list: MOCK_PARTY_LIST_URI,
        })
      }
    }
  }
