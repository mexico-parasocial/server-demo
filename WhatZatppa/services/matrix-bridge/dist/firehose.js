...
export class FirehoseConsumer {
    ...
    constructor(config, db, matrix, metrics, log) {
        this.db = db;
        this.matrix = matrix;
    ...
    async start() {
        this.log.info({ url: this.firehose.opts.service }, 'Starting firehose consumer');
    ...
    async handleCommit(evt) {
        ...
        else if (collection === 'com.para.community.proposal' &&
            evt.event === 'create') {
            await this.handleProposalCreate(did, evt.record);
        }
        else if (collection === 'com.para.community.vote' &&
            evt.event === 'create') {
            await this.handleVoteCast(did, evt.record);
        }
    }
    async handleCommunityCreate(creatorDid, record) {
        if (!record)
            return;
        const slug = (record.slug ?? 'community');
        const name = (record.name ?? 'Unnamed Community');
        const chamberMode = (record.chamberMode ?? 'unicameral');
        ...
        try {
            ...
            this.log.info({ communityUri, spaceId, name, chamberMode }, 'Created Matrix space for community');
            ...
            if (chamberMode === 'bicameral') {
                const [chamberA, chamberB, observerRoom] = await Promise.all([
                    this.matrix.createRoom(`${name} — Cámara A`, `${slug}-chamber-a`, spaceId),
                    this.matrix.createRoom(`${name} — Cámara B`, `${slug}-chamber-b`, spaceId),
                    this.matrix.createRoom(`${name} — Consejo Observador`, `${slug}-observers`, spaceId),
                ]);
                this.db.setChamberRooms(communityUri, chamberA, chamberB, observerRoom);
                this.log.info({ communityUri, chamberA, chamberB, observerRoom }, 'Created bicameral chamber rooms');
            ...
            this.log.error({ err, communityUri }, 'Failed to create Matrix space for community');
            this.db.logSync('create_space', communityUri, creatorDid, null, false, err.message);
        }
        finally {
            end();
        }
    }
    async handleMembershipChange(userDid, record, action) {
        if (!record)
            return;
        const communityUri = record.community;
        const state = record.membershipState;
        ...
        if (!space) {
            this.log.debug({ communityUri }, 'No Matrix space found for community, skipping membership sync');
            return;
        }
        ...
        try {
            if (state === 'active' && (action === 'create' || action === 'update')) {
                ...
                if (space.chamberMode === 'bicameral' && !isObserver) {
                    await this.handleBicameralInvite(communityUri, space, userDid, userMxid, roles);
                }
                    ...
                    if (!alreadyThere) {
                        ...
                        this.log.info({ communityUri, userDid, spaceId: space.spaceId }, 'Invited user to Matrix space');
                    }
                }
                let powerLevel = 0;
            ...
            else if ((state === 'left' || state === 'removed' || state === 'blocked') &&
                action === 'update') {
            ...
            this.log.error({ err, communityUri, userDid, state }, 'Failed to sync membership to Matrix');
            this.db.logSync(eventType, communityUri, userDid, space.spaceId, false, err.message);
        }
        finally {
            end();
        }
    }
    async handleBicameralInvite(communityUri, space, userDid, userMxid, roles) {
        ...
        if (!chamber) {
            ...
            try {
                const proof = await assignChamberVerifiable(userDid, communityUri, countA, countB);
                ...
                this.log.info({ communityUri, userDid, chamber, drandRound: proof.round }, 'Assigned user to chamber via verifiable sortition (drand)');
            }
            catch (err) {
                ...
                this.log.warn({ err: err.message, communityUri, userDid, chamber }, 'drand failed, using fallback sortition');
            }
            this.db.setChamberAssignment(communityUri, userDid, chamber);
        }
        const chamberRoomId = chamber === 'A' ? space.chamberA_RoomId : space.chamberB_RoomId;
        if (!chamberRoomId) {
        ...
        if (!chamberMembers.some((m) => m.user_id === userMxid)) {
            ...
            this.log.info({ communityUri, userDid, chamber, roomId: chamberRoomId }, 'Invited user to chamber');
        }
    ...
    async handleConstitutionUpdate(did, record) {
        if (!record)
            return;
        try {
            const constitution = parseConstitution(record);
            this.db.setConstitution(constitution.community, constitution.version, JSON.stringify(constitution.rules));
            this.log.info({ community: constitution.community, version: constitution.version }, 'Constitution updated');
        }
        catch (err) {
            this.log.error({ err, did, record }, 'Failed to parse constitution');
        }
    }
    async handleProposalCreate(did, record) {
        if (!record)
            return;
        const communityUri = record.community;
        const title = (record.title ?? 'Untitled');
        const body = (record.body ?? '');
        const proposalType = (record.type ?? 'general');
        const budgetRequest = typeof record.budgetRequest === 'number' ? record.budgetRequest : null;
        const createdAt = (record.createdAt ?? new Date().toISOString());
        const uri = `at://${did}/com.para.community.proposal/${Date.now()}`;
        this.proposals.onProposalCreated(uri, communityUri, did, title, body, proposalType, budgetRequest, createdAt);
    }
    async handleVoteCast(did, record) {
        if (!record)
            return;
        const proposalUri = record.proposal;
        const communityUri = record.community;
        const choice = record.choice;
        const createdAt = (record.createdAt ?? new Date().toISOString());
        const uri = `at://${did}/com.para.community.vote/${Date.now()}`;
        this.proposals.onVoteCast(uri, proposalUri, communityUri, did, choice, createdAt);
    }
    async handleObserverInvite(communityUri, space, userMxid) {
        ...
        if (space.chamberA_RoomId) {
            const chamberAMembers = await this.matrix.getRoomMembers(space.chamberA_RoomId);
        ...
        if (space.chamberB_RoomId) {
            const chamberBMembers = await this.matrix.getRoomMembers(space.chamberB_RoomId);
        ...
        if (space.observerRoomId) {
            const observerMembers = await this.matrix.getRoomMembers(space.observerRoomId);
            if (!observerMembers.some((m) => m.user_id === userMxid)) {
                await this.matrix.inviteUser(space.observerRoomId, userMxid);
            }
            this.log.info({ communityUri, userMxid }, 'Invited observer to observer room');
        }
    }
    async kickFromAllRooms(space, userMxid, reason) {
        const rooms = [
            space.spaceId,
        ...
        for (const roomId of rooms) {
            try {
                await this.matrix.kickUser(roomId, userMxid, `Membership state: ${reason}`);
            }
            catch (err) {
                // User might not be in this room, ignore
            }
        }
        this.log.info({ userMxid, state: reason }, 'Removed user from all community rooms');
    }
    async ensureUserExists(mxid, did) {
