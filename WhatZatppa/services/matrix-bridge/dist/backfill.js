...
async function main() {
    ...
    if (communitiesFileIndex >= 0) {
        ...
        log.info({ appviewUrl, communityUri }, 'Fetching single community from AppView');
        try {
        ...
        log.info('  npx tsx src/backfill.ts --appview https://api.para.social --community-uri at://...');
        log.info('');
    ...
    for (const comm of communities) {
        ...
        if (existing) {
            log.info({ uri: comm.uri }, 'Community already has Matrix space, skipping creation');
        }
        else {
            try {
                const spaceId = await matrix.createSpace(comm.name, comm.slug);
                db.setSpaceForCommunity(comm.uri, spaceId, comm.slug, comm.chamberMode ?? 'unicameral');
                ...
                if (comm.chamberMode === 'bicameral') {
                    const [chamberA, chamberB, observerRoom] = await Promise.all([
                        matrix.createRoom(`${comm.name} — Cámara A`, `${comm.slug}-chamber-a`, spaceId),
                        matrix.createRoom(`${comm.name} — Cámara B`, `${comm.slug}-chamber-b`, spaceId),
                        matrix.createRoom(`${comm.name} — Consejo Observador`, `${comm.slug}-observers`, spaceId),
                    ]);
                    db.setChamberRooms(comm.uri, chamberA, chamberB, observerRoom);
                    log.info({ uri: comm.uri, chamberA, chamberB, observerRoom }, 'Created chamber rooms');
                    await Promise.all([
        ...
        log.info('To backfill members, query your AppView for all active memberships and call:');
        log.info('  ensureUser(matrix, db, member.did, serverName)');
        log.info('  matrix.inviteUser(spaceId, mxid)');
        log.info('  For bicameral: assignChamberBalanced() then invite to chamber room');
    }
    db.close();
    log.info('Backfill complete');
}
function ensureUser(matrix, db, did, serverName) {
