...
import { computeAssemblySortitionHash, fetchBeacon, fetchLatestBeacon, } from './drand.js';
...
function formatSortitionRun(row) {
    if (!row)
        return null;
    return {
        id: row.id,
...
function formatSortitionCandidate(row) {
    if (!row)
        return null;
    return {
        runId: row.run_id,
...
async function main() {
    ...
    const processSortitionRun = async (runId) => {
        ...
        const ranked = eligible
            .map((member) => {
            const proof = computeAssemblySortitionHash(member.did, run.community_uri, run.cabildeo_uri, beacon);
            return {
                did: member.did,
        ...
        await sendExpoNotifications({
            tokens: pushTokens
                .map((token) => token.expoPushToken ?? token.expo_push_token)
                .filter(Boolean),
    ...
    const processScheduledSortitionRuns = async () => {
        const scheduled = db.getScheduledSortitionRuns(10);
        for (const run of scheduled) {
            try {
                ...
                if (!message.includes('drand fetch failed')) {
                    log.warn({ err, runId: run.id }, 'Scheduled sortition run did not process');
                }
            }
        }
    };
        ...
        try {
            if (req.url === '/healthz') {
                ...
                if (mxid) {
                    try {
                        const tokenData = await matrix.generateUserToken(mxid);
                        await matrix.setPusherWithUserToken(mxid, tokenData.accessToken, expoPushToken, 'com.para.app', gatewayUrl);
            ...
            else if (req.url === '/_matrix/push/v1/notify' &&
                req.method === 'POST') {
                const body = await readBody(req);
                ...
                const senderMxid = notification.sender;
                ...
                if (pushTokens.length > 0) {
                    try {
                        ...
                        log.debug({ roomId, tokens: pushTokens.length }, 'Sent push notification');
                    }
                    catch (err) {
                ...
                const body = await readBody(req);
                const { cabildeoUri, communityUri, createdByDid, assemblySize = 100, eligibilityFilter = 'all', drandRound, roundOffset = 20, } = JSON.parse(body);
                ...
                const targetRound = Number(drandRound) > 0
                    ? Number(drandRound)
            ...
            else if (req.url?.startsWith('/api/sortition/runs') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                ...
                const run = db.getSortitionRunByCabildeo(cabildeoUri);
                if (!run) {
                ...
                const viewerCandidate = viewerDid
                    ? formatSortitionCandidate(db.getSortitionCandidate(run.id, viewerDid))
                    : null;
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({
                    run: formatSortitionRun(run),
            ...
            else if (req.url === '/api/sortition/runs/process' &&
                req.method === 'POST') {
                const body = await readBody(req);
            ...
            else if (req.url?.startsWith('/api/sortition-proofs') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
            ...
            else if (req.url?.startsWith('/api/sortition-proof') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                ...
                const matches = stored.drand_round === round &&
            ...
            else if (req.url?.startsWith('/api/sortition-proof-as-record') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const did = url.searchParams.get('did');
                const communityUri = url.searchParams.get('community');
            ...
            else if (req.url?.startsWith('/api/constitution') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('uri');
                if (!communityUri) {
            ...
            else if (req.url?.startsWith('/api/proposals') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('community');
            ...
            else if (req.url?.startsWith('/api/decisions') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('community');
                if (!communityUri) {
            ...
            else if (req.url?.startsWith('/api/chat-badges') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const did = url.searchParams.get('did');
                const communityUri = url.searchParams.get('community');
            ...
            else if (req.url?.startsWith('/api/chat-member-list') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('community');
                if (!communityUri) {
            ...
            else if (req.url === '/api/moderation-report' &&
                req.method === 'POST') {
                const body = await readBody(req);
                const { reportedDid, reporterDid, communityUri, reason, context, matrixEventId, matrixRoomId, } = JSON.parse(body);
                ...
                const reporterStats = db.getParticipationStats(reporterDid, communityUri);
                if (!reporterStats) {
                    res.writeHead(403, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ error: 'Reporter is not a community member' }));
                    return;
                }
            ...
            else if (req.url === '/api/moderation-sanction' &&
                req.method === 'POST') {
                const body = await readBody(req);
                const { targetDid, sanctionedByDid, communityUri, type, durationMinutes, matrixRoomId, } = JSON.parse(body);
                ...
                if (!modStats || !modStats.is_moderator) {
                    res.writeHead(403, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ error: 'Only moderators can apply sanctions' }));
                    return;
                }
            ...
            else if (req.url?.startsWith('/api/moderation-dashboard') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('community');
                ...
                if (!modStats || !modStats.is_moderator) {
                    res.writeHead(403, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ error: 'Only moderators can view dashboard' }));
                    return;
                }
            ...
            else if (req.url === '/api/moderation-recompute' &&
                req.method === 'POST') {
                const body = await readBody(req);
            ...
            else if (req.url?.startsWith('/api/user-chat-preferences') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const did = url.searchParams.get('did');
                if (!did) {
            ...
            else if (req.url === '/api/user-chat-preferences' &&
                req.method === 'POST') {
                const body = await readBody(req);
                ...
                const body = await readBody(req);
                const { communityUri, authorDid, title, content, cardType, sourceUrl, isPublic, passportVisible, metadata, } = JSON.parse(body);
            ...
            else if ((req.url === '/api/community-tree/contributions' ||
                req.url === '/api/community-map/contributions') &&
                req.method === 'POST') {
                const body = await readBody(req);
                const { communityUri, authorDid, title, content, sourceUrl, sourceType, metadata, } = JSON.parse(body);
            ...
            else if ((req.url?.startsWith('/api/community-tree/contributions') ||
            ...
            else if ((req.url === '/api/community-tree/contributions/vote' ||
                ...
                const contribution = db.voteCommunityMapContribution(contributionId, voterDid, vote);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ contribution }));
            }
            else if (req.url === '/api/relationships' && req.method === 'POST') {
                const body = await readBody(req);
                const { sourceCardId, targetCardId, relationshipType, authorDid } = JSON.parse(body);
            ...
            else if (req.url?.startsWith('/api/suggestions') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('community');
                const status = url.searchParams.get('status') || 'pending';
            ...
            else if (req.url === '/api/suggestions/accept' &&
                req.method === 'POST') {
                const body = await readBody(req);
            ...
            else if (req.url === '/api/suggestions/reject' &&
                req.method === 'POST') {
                const body = await readBody(req);
            ...
            else if (req.url?.startsWith('/api/summarize') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('community');
                if (!communityUri) {
                ...
                const client = new OpenAIClient(config.openaiApiKey, config.openaiModel || 'gpt-4o-mini');
                const summary = await summarizeCommunityDeliberation(client, db, communityUri);
                if (!summary) {
                    res.writeHead(200, { 'Content-Type': 'application/json' });
                ...
                const totalInfluence = votes.reduce((sum, v) => sum + v.influence, 0);
                res.writeHead(200, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({
                    success: true,
                    totalInfluence,
            ...
            else if (req.url?.startsWith('/api/community-pulse') &&
                req.method === 'GET') {
                const url = new URL(req.url, `http://localhost:${config.port}`);
                const communityUri = url.searchParams.get('community');
    ...
    const proposalCron = setInterval(() => {
        proposals.processStateTransitions().catch((err) => {
