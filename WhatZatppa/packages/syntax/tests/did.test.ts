...
import { InvalidDidError, ensureValidDid, ensureValidDidRegex } from '../src/index.js'
...
describe('DID permissive validation', () => {
  ...
  it('conforms to interop valid DIDs', () => {
    const lineReader = readline.createInterface({
      input: fs.createReadStream(
        `${import.meta.dirname}/interop-files/did_syntax_valid.txt`,
      ),
      terminal: false,
    })
  ...
  it('conforms to interop invalid DIDs', () => {
    const lineReader = readline.createInterface({
      input: fs.createReadStream(
        `${import.meta.dirname}/interop-files/did_syntax_invalid.txt`,
      ),
      terminal: false,
    })
    lineReader.on('line', (line) => {
