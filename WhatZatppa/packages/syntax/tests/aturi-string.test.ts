...
import { InvalidAtUriError, assertAtUriString, isAtUriString } from '../src/index.js'

describe('valid interop', () => {
  for (const value of readLines(
    `${import.meta.dirname}/../../../interop-test-files/syntax/aturi_syntax_valid.txt`,
  )) {
    testValid(value)
  }
})
...
describe('invalid interop', () => {
  for (const value of readLines(
    `${import.meta.dirname}/../../../interop-test-files/syntax/aturi_syntax_invalid.txt`,
  )) {
    testInvalid(value)
  }
})
